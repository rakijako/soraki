...
async function gql(query, variables = {}) {
  ...
  console.log("RAW API:", JSON.stringify(data).slice(0, 500));
  return data;
}
...
export default function SorareSO5() {
  ...
  const run = useCallback(async (currentSlug, filter) => {
    ...
    try {
      ...
      if (!data?.data?.user) {
        const errMsg = data?.errors?.[0]?.message || "Slug introuvable — vérifie ton pseudo Sorare.";
